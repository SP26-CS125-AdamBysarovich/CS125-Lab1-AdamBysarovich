...
class BankAccount:
    ...
    def transfer(self, amount, other_account):
        if amount > self.balance:
            return "insufficient funds!"
        if other_account == True:
            input("Enter the name of the account you want to transfer to? ")
            amount = int("how much money do you want to transfer? ")
        if amount > 0:
            self.balance -= amount
            other_account.balance += amount
            return f"{amount} transfered to {other_account}"

    def apply_interest(self, rate):
        if self == True:
            rate= int("how much is the interest rate? ")
        if rate >0:
            answer=self.balance*rate
            return answer
# DEMONSTRATION
